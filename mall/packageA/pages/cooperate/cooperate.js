    ...
    if (that.data.type) {
      app.Util.ajax(`mall/home/activity/freeShopping?mode=${2}&type=${that.data.type}`, null, 'GET').then((res) => {
        if (res.data.content) {
          ...
          }else if (res.data.content.type == 3) {
            that.setData({
              btnText: '免费领取'
            })
          that.setData({
      ...
      if (res.data.messageCode == 'MSG_1001') {
        ...
        wx.getImageInfo({
          ...
          success(res) {
            ...
            ctx.fillText('我的邀请码：' + app.globalData.cooperateInvitionCode, 0.442 * width, 120);
            ctx.stroke();
            ctx.closePath()
            // 绘制最小矩形
            ctx.beginPath()
    ...
    if (ops.from === 'button') {
      if (ops.target.id === 'btn') {
        ...
        return {
          ...
          path: "/packageA/pages/cooperate/cooperate?inviterCode=" + app.globalData.cooperateInvitionCode,
