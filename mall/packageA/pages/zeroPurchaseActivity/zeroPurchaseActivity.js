  ...
  showBtn: function(e) {
    ...
    if (that.data.type == 4) {
      app.Util.ajax('mall/home/activity/freeShopping/obtaining/validate', {
        ...
        if (res.data.messageCode == 'MSG_1001') {
          wx.navigateTo({
            url: `/packageA/pages/zeroPurchase/zeroPurchase?orgPrice=${e.currentTarget.dataset.price}&id=${e.currentTarget.dataset.goodsid}&type=${that.data.type}&text=${e.currentTarget.dataset.text}`
          })
        } else {
          that.setData({
            showText: res.data.message
        ...
        if (res.data.messageCode == 'MSG_1001') {
          ...
          wx.getImageInfo({
            ...
            success(res) {
              ...
              // 绘制广告语
              ctx.beginPath()
              var adTips = '【新品免费体验】我是合伙人，快来免费尝鲜，保证能领到'
              ctx.setFontSize(14);
        ...
        if (res.data.messageCode == 'MSG_1001') {
          ...
          wx.getImageInfo({
            ...
            success(res) {
              ...
              // 绘制广告语
              ctx.beginPath()
              var adTips = '优质新品免费体验，仅限信用卡用户！我已领，分享给你'
              ctx.setFontSize(14);
              ctx.setFillStyle('#333333');
              ctx.setTextAlign("left")
              let chr = adTips.split('') // 分割为字符串数组
      ...
      if (ops.target.id === 'btn') {
        ...
        if (that.data.type == 1) {
          ...
          return {
            ...
            path: "/packageA/pages/zeroPurchaseActivity/zeroPurchaseActivity?inviterCode=" + wx.getStorageSync('inviterCode') + "&&type=" + 1,
          ...
          return {
            title: '优质新品免费体验，仅限信用卡用户！我已领，分享给你',
            path: "/packageA/pages/zeroPurchaseActivity/zeroPurchaseActivity?inviterCode=" + wx.getStorageSync('inviterCode') + "&&type=" + 2,
            imageUrl: '/packageA/img/xuncaoji_cheats.png',
          }
        } else if (that.data.type == 4) {
          app.Util.ajax('mall/weChat/sharing/onSuccess', {
          ...
          return {
            ...
            path: "/packageA/pages/zeroPurchaseActivity/zeroPurchaseActivity?inviterCode=" + wx.getStorageSync('inviterCode') + "&&type=" + 4,
            imageUrl: '/packageA/img/xuncaoji_cheats.png',
          }
        }
        ...
        if (that.data.type == 1) {
          ...
          return {
            title: '群福利~优质新品免费体验，仅限信用卡用户！我已领，真实可靠！',
            path: "/packageA/pages/zeroPurchaseActivity/zeroPurchaseActivity?inviterCode=" + wx.getStorageSync('inviterCode') + "&&type=" + 2,
            imageUrl: '/packageA/img/xuncaoji_cheats.png',
          }
        } else if (that.data.type == 4) {
