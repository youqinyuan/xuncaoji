...
Page({
  ...
  data: {
    ...
    causeValue2: {
      text1: '当前商品您已终止与平台的“一折购”合约，系统已自动终止剩余返现，返现详细信息请前往“我的订单”页面查看。',
      text2: '如有疑问请联系平台客服。'
    },
    //返现原因分类
    causeStatue: 1,
  ...
  onLoad: function(options) {
    ...
    that.setData({
      ...
      goMentionPeriod: options.goMentionPeriod ? options.goMentionPeriod:null
    })
    if (options.temp) {
      setTimeout(function() {
        wx.showToast({
    ...
    app.Util.ajax('mall/personal/queryPendingReturnDetails', {
      ...
      if (res.data.content.noReturnItem) {
        ...
        that.setData({
          ...
          noWithdrawalAmount: res.data.content.noWithdrawalAmount
        })
      ...
      if (res.data.content) {
        ...
        for (var i = 0; i < res.data.content.noReturnItem.items.length; i++) {
          ...
          if (res.data.content.noReturnItem.items[i].otherTransfer == 1) {
            ...
            res.data.content.noReturnItem.items[i].remark = "购买—一折购提期"
            res.data.content.noReturnItem.items[i].proStatus = 1
          }
      ...
      if (res.data.content) {
        ...
        for (var i = 0; i < res.data.content.items.length; i++) {
          ...
          if (res.data.content.items[i].otherTransfer == 1) {
            ...
            res.data.content.items[i].remark = "购买—一折购提期"
